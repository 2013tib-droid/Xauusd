...
rulebook, backtester, dan briefing pra-entry. Backtester punya dua wajah — CLI Python
di `xauusd/`, dan port JavaScript di `web/` yang dipublikasikan sebagai GitHub Pages.
...
node web/tests/parity.mjs     # hasil JS harus sama persis dengan hasil Python
```
...
- **`web/` adalah port, bukan implementasi kedua.** Setiap perubahan di `xauusd/`
  yang menyentuh indikator, strategi, engine, atau metrik harus diikuti
  `python3 web/tests/make_fixture.py` lalu commit hasilnya, dan `node web/tests/parity.mjs`
  harus tetap hijau. Backtester web yang angkanya berbeda tipis dari CLI lebih berbahaya
  daripada yang jelas-jelas rusak: keduanya sama-sama terlihat masuk akal. CI menolak
  fixture yang kedaluwarsa justru karena itu.
